...
class DBTests:
  ...
  def testEmployeeTableFunctions =
    ...
    assertEquals(
      List(e1, e3),
      EmployeeTable.find(Option("e1") -> None)
    )

    assertEquals(
      List(e2, e3),
      EmployeeTable.find(None -> Option(2d))
    )

    assertEquals(
      List(e1),
      EmployeeTable.find(Option("e1") -> Option(1d))
    )

    assertEquals(
      List(),
      EmployeeTable.find(None -> None)
    )

    assertEquals(
      List(),
      EmployeeTable.find(Option("e123") -> None)
    )
  ...
  def testManagerTableFunctions =
    ...
    assertEquals(
      List(m1, m3),
      ManagerTable.find(Option("m1") -> None)
    )

    assertEquals(
      List(m2, m3),
      ManagerTable.find(None -> Option("p2"))
    )

    assertEquals(
      List(m1),
      ManagerTable.find(Option("m1") -> Option("p1"))
    )

    assertEquals(
      List(),
      ManagerTable.find(None -> None)
    )

    assertEquals(
      List(),
      ManagerTable.find(Option("m123") -> None)
    )
  ...
  def testTaskTableFunctions =
    ...
    assertEquals(
      List(t1, t3),
      TaskTable.find(Option("t1"))
    )

    assertEquals(
      List(t0),
      TaskTable.find(Option("t0"))
    )
